!> \file
!> $Id: CylinderInflationDrivenDarcyExample.f90 20 2007-05-28 20:22:52Z cpb $
...
!   INTEGER(CMISSIntg), PARAMETER :: FieldAnalyticUserNumber=1337
  ...
  DYNAMIC_SOLVER_DARCY_STOP_TIME=450_CMISSIntg * DYNAMIC_SOLVER_DARCY_TIME_INCREMENT
...
!   CALL CMISSEquationsSetBoundaryConditionsAnalytic(EquationsSetSolid,Err)
  ...
  CALL CMISSEquationsSetBoundaryConditionsCreateStart(EquationsSetDarcy,BoundaryConditionsDarcy,Err)
    ...
    DO NN=1,SIZE(BottomSurfaceNodesDarcyVel,1)
      VALUE = 0.0_CMISSDP
      COMPONENT_NUMBER = 3
      CALL CMISSBoundaryConditionsSetNode(BoundaryConditionsDarcy,CMISSFieldVVariableType,1,BottomSurfaceNodesDarcyVel(NN), &
        & COMPONENT_NUMBER,CMISSBoundaryConditionFixed,VALUE,Err)
