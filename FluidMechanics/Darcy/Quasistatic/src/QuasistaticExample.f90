...
! ! 
! !  This example considers time-dependent material parameters and
! !  and boundary conditions and, thus, we solve it as quasi-static problem.
! ! 
...
!---------------
  ...
  INTEGER:: INDEPENDENT_FIELD_USER_NUMBER
...
!-----------------
  ...
  CALL EQUATIONS_SET_CREATE_START(1,REGION,GEOMETRIC_FIELD,EQUATIONS_SET,ERR,ERROR,*999)
    CALL EQUATIONS_SET_SPECIFICATION_SET(EQUATIONS_SET,EQUATIONS_SET_FLUID_MECHANICS_CLASS,EQUATIONS_SET_DARCY_EQUATION_TYPE, &
      & EQUATIONS_SET_QUASISTATIC_DARCY_SUBTYPE,ERR,ERROR,*999)
  ...
  CALL EQUATIONS_SET_MATERIALS_CREATE_START(EQUATIONS_SET,MATERIALS_FIELD_USER_NUMBER,MATERIALS_FIELD, &
    ...
    CALL FIELD_COMPONENT_INTERPOLATION_SET(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE,1,FIELD_NODE_BASED_INTERPOLATION,ERR,ERROR,*999)

    CALL FIELD_COMPONENT_INTERPOLATION_SET(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE,2,FIELD_NODE_BASED_INTERPOLATION,ERR,ERROR,*999)
...
!     & FIELD_VALUES_SET_TYPE,3,k_zz,ERR,ERROR,*999)
  ...
  !----------------------------------------------------
  ! Create the equations_set INdependent_field_variables
  !----------------------------------------------------

  NULLIFY(INDEPENDENT_FIELD)

  INDEPENDENT_FIELD_USER_NUMBER = 4

  CALL EQUATIONS_SET_INDEPENDENT_CREATE_START(EQUATIONS_SET,INDEPENDENT_FIELD_USER_NUMBER,INDEPENDENT_FIELD, &
    & ERR,ERROR,*999)
  CALL EQUATIONS_SET_INDEPENDENT_CREATE_FINISH(EQUATIONS_SET,ERR,ERROR,*999)


  !--------------------------------------------------------
  ! Initialise the equations_set INdependent_field_variables
  !--------------------------------------------------------
...
!     CALL EQUATIONS_OUTPUT_TYPE_SET(EQUATIONS,EQUATIONS_ELEMENT_MATRIX_OUTPUT,ERR,ERROR,*999)
  ...
  CALL PROBLEM_CREATE_START(1,PROBLEM,ERR,ERROR,*999)
    CALL PROBLEM_SPECIFICATION_SET(PROBLEM,PROBLEM_FLUID_MECHANICS_CLASS,PROBLEM_DARCY_EQUATION_TYPE, &
      & PROBLEM_QUASISTATIC_DARCY_SUBTYPE,ERR,ERROR,*999)
  ...
  CALL PROBLEM_CONTROL_LOOP_CREATE_START(PROBLEM,ERR,ERROR,*999)
    CALL PROBLEM_CONTROL_LOOP_GET(PROBLEM,CONTROL_LOOP_NODE,CONTROL_LOOP,ERR,ERROR,*999)
      CALL CONTROL_LOOP_TIMES_SET(CONTROL_LOOP,-1.0_DP,4.0_DP,1.0_DP,ERR,ERROR,*999)
