!> \file
!> $Id: TransientExample.f90 20 2009-04-08 20:22:52Z cpb $
...
!> \example FluidMechanics/Stokes/TESTING_ELEMENTS/quad/Cubic/IterativeSolver/Transient/src/TransientExample.f90
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  TYPE(SOLVER_TYPE), POINTER :: DYNAMIC_SOLVER, LINEAR_SOLVER
...
!   CHARACTER(LEN=MAXSTRLEN) :: DIAG_ROUTINE_LIST(1),TIMING_ROUTINE_LIST(1)
  ...
  REAL(DP) :: DIVERGENCE_TOLERANCE, RELATIVE_TOLERANCE, ABSOLUTE_TOLERANCE, LINESEARCH_ALPHA
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IF(CM%D==2) THEN
    CALL BASIS_QUADRATURE_NUMBER_OF_GAUSS_XI_SET(BASIS_M,(/3,3/),ERR,ERROR,*999)
  ...
  IF(CM%D==2) THEN
    CALL BASIS_QUADRATURE_NUMBER_OF_GAUSS_XI_SET(BASIS_V,(/3,3/),ERR,ERROR,*999)
  ...
  IF(CM%D==2) THEN
    CALL BASIS_QUADRATURE_NUMBER_OF_GAUSS_XI_SET(BASIS_P,(/3,3/),ERR,ERROR,*999)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  CALL EQUATIONS_SET_SPECIFICATION_SET(EQUATIONS_SET,EQUATIONS_SET_FLUID_MECHANICS_CLASS, &
    & EQUATIONS_SET_STOKES_EQUATION_TYPE,EQUATIONS_SET_TRANSIENT_STOKES_SUBTYPE,ERR,ERROR,*999)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  BC_NUMBER_OF_WALL_NODES=14
  ALLOCATE(BC_WALL_NODES(BC_NUMBER_OF_WALL_NODES))
  BC_WALL_NODES=(/1,3,7,11,17,18,23,26,29,31,35,41,44,47/)

  BC_NUMBER_OF_INLET_NODES=5
  ALLOCATE(BC_INLET_NODES(BC_NUMBER_OF_INLET_NODES))
!  BC_INLET_NODES=(/30/)
   BC_INLET_NODES=(/30,39,40,48,49/)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  CALL PROBLEM_SPECIFICATION_SET(PROBLEM,PROBLEM_FLUID_MECHANICS_CLASS,PROBLEM_STOKES_EQUATION_TYPE, &
    & PROBLEM_TRANSIENT_STOKES_SUBTYPE,ERR,ERROR,*999)
  ...
  CALL PROBLEM_SOLVER_GET(PROBLEM,CONTROL_LOOP_NODE,1,DYNAMIC_SOLVER,ERR,ERROR,*999)
...
!   CALL SOLVER_OUTPUT_TYPE_SET(SOLVER,SOLVER_PROGRESS_OUTPUT,ERR,ERROR,*999)
  CALL SOLVER_DYNAMIC_THETA_SET(DYNAMIC_SOLVER,2.0_DP/3.0_DP,ERR,ERROR,*999)

  !Get the dynamic linear solver
  CALL SOLVER_DYNAMIC_LINEAR_SOLVER_GET(DYNAMIC_SOLVER,LINEAR_SOLVER,ERR,ERROR,*999)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  CALL PROBLEM_SOLVER_GET(PROBLEM,CONTROL_LOOP_NODE,1,DYNAMIC_SOLVER,ERR,ERROR,*999)
  CALL SOLVER_SOLVER_EQUATIONS_GET(DYNAMIC_SOLVER,SOLVER_EQUATIONS,ERR,ERROR,*999)
