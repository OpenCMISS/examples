...
!> \example FluidMechanics/Darcy/Dynamic/src/DynamicExample.f90
!! Example program to solve a dynamic Darcy equation using OpenCMISS calls.
!! \par Latest Builds:
!! \li <a href='http://autotest.bioeng.auckland.ac.nz/opencmiss-build/logs_x86_64-linux/FluidMechanics/Darcy/Dynamic/build-intel'>Linux Intel Build</a>
!! \li <a href='http://autotest.bioeng.auckland.ac.nz/opencmiss-build/logs_x86_64-linux/FluidMechanics/Darcy/Dynamic/build-intel'>Linux GNU Build</a>
!!
!<

! ! 
! !  This example considers a dynamic Darcy problem.
! ! 
...
!   INTEGER(CMISSIntg) :: MPI_IERROR
  ...
  DYNAMIC_SOLVER_DARCY_OUTPUT_TYPE=CMISSSolverNoOutput
  ...
  DYNAMIC_SOLVER_DARCY_STOP_TIME=5.0_CMISSDP 
  DYNAMIC_SOLVER_DARCY_TIME_INCREMENT=1.0_CMISSDP
...
!   CALL CMISSDiagnosticsSetOn(CMISSInDiagType,DIAG_LEVEL_LIST,"DarcyDiagnostics",DIAG_ROUTINE_LIST,Err)
  ...
  DO NODE_NUMBER=1,NUMBER_OF_NODES_GEOMETRY
    DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
      CALL CMISSDecompositionNodeDomainGet(Decomposition,NODE_NUMBER,MESH_COMPONENT_NUMBER_GEOMETRY,NodeDomain,Err)
...
CALL CMISSEquationsSetCreateStart(EquationsSetUserNumberDarcy,Region,GeometricField,CMISSEquationsSetFluidMechanicsClass, &
    & CMISSEquationsSetDarcyEquationType,CMISSEquationsSetTransientDarcySubtype,EquationsSetFieldUserNumber,EquationsSetField, &
  ...
  IF( CM%D==2_CMISSIntg ) THEN
    DO NODE_NUMBER=1_CMISSIntg,NUMBER_OF_NODES_GEOMETRY
      ...
      IF( (ABS(COORD_X-DOMAIN_X1) < GEOMETRY_TOLERANCE) .AND. ComputationalNodeNumber==NodeDomain ) THEN
        !x-velocity
        VALUE = 1.0_CMISSDP
        CALL CMISSBoundaryConditionsSetNode(BoundaryConditionsDarcy,CMISSFieldUVariableType,1,CMISSNoGlobalDerivative, & 
  ...
  CALL CMISSProblemSolverGet(Problem,CMISSControlLoopNode,SolverDarcyUserNumber,DynamicSolverDarcy,Err)
  !Set the output type
...
!   CALL CMISSProblemSolverGet(Problem,CMISSControlLoopNode,SolverDarcyUserNumber,LinearSolverDarcy,Err)
  ...
  CALL CMISSProblemSolverGet(Problem,CMISSControlLoopNode,SolverDarcyUserNumber,DynamicSolverDarcy,Err)
  CALL CMISSSolverSolverEquationsGet(DynamicSolverDarcy,SolverEquationsDarcy,Err)
  ...
  IF(EXPORT_FIELD_IO) THEN
    ...
    CALL CMISSFieldIONodesExport(Fields,"DynamicDarcy","FORTRAN",Err)
    CALL CMISSFieldIOElementsExport(Fields,"DynamicDarcy","FORTRAN",Err)
