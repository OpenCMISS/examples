...
!   TYPE(CMISSBoundaryConditionsType) :: BoundaryConditionsStokes
  ...
  DYNAMIC_SOLVER_STOKES_STOP_TIME=0.03000001_CMISSDP
  DYNAMIC_SOLVER_STOKES_TIME_INCREMENT=0.010_CMISSDP
  DYNAMIC_SOLVER_STOKES_THETA=1.0_CMISSDP/1.0_CMISSDP
...
!   READ(*,*) 
  ...
  IF(INPUT_TYPE==5.AND.NUMBER_OF_DIMENSIONS==2) THEN
    ANALYTICAL_TYPE=CMISSEquationsSetStokesTwoDim5
  ELSE IF(INPUT_TYPE==5.AND.NUMBER_OF_DIMENSIONS==3) THEN
    ANALYTICAL_TYPE=CMISSEquationsSetStokesThreeDim5
  ELSE
    !TODO more detailed error message
    WRITE(*,*)'INPUT ERROR!!!'
  ENDIF
