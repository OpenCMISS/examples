...
!  REAL(CMISSRP), PARAMETER :: PERIODD=10.00_CMISSRP
  REAL(CMISSRP), PARAMETER :: PERIODD=20.00_CMISSRP
!  REAL(CMISSRP), PARAMETER :: TIME_STOP=300.0_CMISSRP
  REAL(CMISSRP), PARAMETER :: TIME_STOP=500.0_CMISSRP
  REAL(CMISSRP), PARAMETER :: TIME_STOP_2=750.0_CMISSRP !time the muscle is stimulated at fixed length + stretch
...
!  REAL(CMISSRP), PARAMETER :: Vmax=-0.2_CMISSRP !to stabilise...
  ...
  REAL(CMISSRP), PARAMETER :: alpha=5.0_CMISSRP  !1.0_CMISSRP Original Value  !5.0_CMISSRP
...
!##################################################################################################################################
    filename = "cellModelFiles/slow_TK_2015_06_25.xml"
...
!    CALL cmfe_Field_NumberOfComponentsSet(IndependentFieldM,CMFE_FIELD_U_VARIABLE_TYPE,1,Err)
    ...
    CALL cmfe_Field_ComponentInterpolationSet(IndependentFieldM,CMFE_FIELD_U2_VARIABLE_TYPE,3, &
     & CMFE_FIELD_NODE_BASED_INTERPOLATION,Err)
    CALL cmfe_Field_VariableLabelSet(IndependentFieldM,CMFE_FIELD_U2_VARIABLE_TYPE,"contraction velocity",Err)
...
!  VALUE = VALUE-ABS(Vmax)/10.0_CMISSRP*STIM_STOP
  ...
  DO node_idx=1,SIZE(LeftSurfaceNodes,1)
    ...
    CALL cmfe_Decomposition_NodeDomainGet(DecompositionFE,NodeNumber,1,NodeDomain,Err)
    IF(NodeDomain==ComputationalNodeNumber) THEN
      CALL cmfe_Field_ParameterSetUpdateNode(DependentFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE, &
