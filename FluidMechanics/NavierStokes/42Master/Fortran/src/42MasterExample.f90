...
!> \brief This is a master example program to solve static, dynamic and ALE Navier-Stokes equations using OpenCMISS calls.
!>
!> \section LICENSE
!>
...
!! Example program to solve a static NavierStokes equation using OpenCMISS calls.
...
PROGRAM NAVIERSTOKESMASTEREXAMPLE
...
!   TYPE(cmfe_FieldType) :: EquationsSetField
  ...
  INTEGER(CMFEIntg), PARAMETER :: AnalyticFieldUserNumberNavierStokes=42

  INTEGER(CMFEIntg), PARAMETER :: DomainUserNumber=1
  INTEGER(CMFEIntg), PARAMETER :: SolverNavierStokesUserNumber=1
  !should be 1 and the Navier-Stokes solver 2
...
!   INTEGER(CMFEIntg) :: MPI_IERROR
  ...
  INTEGER(CMFEIntg) :: DYNAMIC_SOLVER_NAVIER_STOKES_INPUT_OPTION=2
  ...
  DYNAMIC_SOLVER_NAVIER_STOKES_FLAG=.FALSE.
  ALE_SOLVER_NAVIER_STOKES_FLAG=.FALSE.
  ANALYTIC_FLAG=.FALSE.
  IF(NUMBER_OF_DIMENSIONS==2) THEN
    ANALYTIC_TYPE=CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_1
  ELSE
    ANALYTIC_TYPE=CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_1
  ENDIF
  OUTPUT_STRING='DEFAULT'
  DYNAMIC_SOLVER_NAVIER_STOKES_START_TIME=0.0_CMFEDP
  DYNAMIC_SOLVER_NAVIER_STOKES_STOP_TIME=3.0_CMFEDP 
  DYNAMIC_SOLVER_NAVIER_STOKES_TIME_INCREMENT=1.0_CMFEDP
  !Set initial boundary conditions
  BOUNDARY_CONDITIONS_NAVIER_STOKES=0.0_CMFEDP
  ...
  IF(NUMBER_OF_DIMENSIONS==2) THEN
    IF(DYNAMIC_SOLVER_NAVIER_STOKES_FLAG) THEN
      ANALYTIC_TYPE=CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_5
    ELSE
      ANALYTIC_TYPE=CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_1
    ENDIF
  ELSE
    IF(DYNAMIC_SOLVER_NAVIER_STOKES_FLAG) THEN
      ANALYTIC_TYPE=CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_5
    ELSE
      ANALYTIC_TYPE=CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_1
    ENDIF
  ENDIF
  ...
  ELSE
    IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_1.AND.NUMBER_OF_DIMENSIONS==2) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_1'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_2.AND.NUMBER_OF_DIMENSIONS==2) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_2'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_3.AND.NUMBER_OF_DIMENSIONS==2) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_3'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_4.AND.NUMBER_OF_DIMENSIONS==2) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_4'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_5.AND.NUMBER_OF_DIMENSIONS==2) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TWO_DIM_5'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_1.AND.NUMBER_OF_DIMENSIONS==3) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_1'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_2.AND.NUMBER_OF_DIMENSIONS==3) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_2'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_3.AND.NUMBER_OF_DIMENSIONS==3) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_3'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_4.AND.NUMBER_OF_DIMENSIONS==3) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_4'
    ELSE IF(ANALYTIC_TYPE==CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_5.AND.NUMBER_OF_DIMENSIONS==3) THEN
      WRITE(*,*)'  -analytictype...', 'CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_THREE_DIM_5'
    ENDIF
  ENDIF
  WRITE(*,*) ' '
  ...
  IF(MOVED_WALL_NODES_NAVIER_STOKES_FLAG) THEN
    ...
    READ(1,*) NUMBER_OF_MOVED_WALL_NODES_NAVIER_STOKES
    ALLOCATE(MOVED_WALL_NODES_NAVIER_STOKES(NUMBER_OF_MOVED_WALL_NODES_NAVIER_STOKES))
    READ(1,*) MOVED_WALL_NODES_NAVIER_STOKES(1:NUMBER_OF_MOVED_WALL_NODES_NAVIER_STOKES)
    CLOSE(1)
  ENDIF
  IF(INLET_WALL_NODES_NAVIER_STOKES_FLAG) THEN
    OPEN(UNIT=1, FILE="./input/bc/FREE_INLET",STATUS='unknown')
    READ(1,*) NUMBER_OF_INLET_WALL_NODES_NAVIER_STOKES
    ALLOCATE(INLET_WALL_NODES_NAVIER_STOKES(NUMBER_OF_INLET_WALL_NODES_NAVIER_STOKES))
    READ(1,*) INLET_WALL_NODES_NAVIER_STOKES(1:NUMBER_OF_INLET_WALL_NODES_NAVIER_STOKES)
    CLOSE(1)
  ENDIF
  ...
  NONLINEAR_SOLVER_NAVIER_STOKES_OUTPUT_TYPE=CMFE_SOLVER_PROGRESS_OUTPUT
  ...
  !Create the equations set for static/dynamic Navier-Stokes
  ...
  !Set the equations set to be a static/dynamic Navier-Stokes problem
  IF(DYNAMIC_SOLVER_NAVIER_STOKES_FLAG) THEN
    IF(ALE_SOLVER_NAVIER_STOKES_FLAG) THEN
      CALL cmfe_EquationsSet_CreateStart(EquationsSetUserNumberNavierStokes,Region,GeometricField, &
        ...
        & CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TYPE,CMFE_EQUATIONS_SET_ALE_NAVIER_STOKES_SUBTYPE], &
          & EquationsSetFieldUserNumber, &
        & EquationsSetFieldNavierStokes,EquationsSetNavierStokes,Err)
    ELSE
      CALL cmfe_EquationsSet_CreateStart(EquationsSetUserNumberNavierStokes,Region,GeometricField, &
        & [CMFE_EQUATIONS_SET_FLUID_MECHANICS_CLASS, &
        & CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TYPE,CMFE_EQUATIONS_SET_TRANSIENT_NAVIER_STOKES_SUBTYPE], &
          & EquationsSetFieldUserNumber, &
        & EquationsSetFieldNavierStokes,EquationsSetNavierStokes,Err)
    ENDIF
  ELSE
    CALL cmfe_EquationsSet_CreateStart(EquationsSetUserNumberNavierStokes,Region,GeometricField, &
      & [CMFE_EQUATIONS_SET_FLUID_MECHANICS_CLASS, &
      & CMFE_EQUATIONS_SET_NAVIER_STOKES_EQUATION_TYPE,CMFE_EQUATIONS_SET_STATIC_NAVIER_STOKES_SUBTYPE], &
        & EquationsSetFieldUserNumber, &
      & EquationsSetFieldNavierStokes,EquationsSetNavierStokes,Err)
  ENDIF
  ...
  !Create the equations set dependent field variables for static/dynamic Navier-Stokes
  ...
  !Create the equations set materials field variables for static/dynamic Navier-Stokes
  ...
  !ANALYTIC FIELDS
  !Create the equations set analytic field variables for static/dynamic Navier-Stokes
    CALL cmfe_Field_Initialise(AnalyticFieldNavierStokes,Err)
    IF(NUMBER_OF_DIMENSIONS==2) THEN  
      CALL cmfe_EquationsSet_AnalyticCreateStart(EquationsSetNavierStokes,ANALYTIC_TYPE,AnalyticFieldUserNumberNavierStokes, &
  ...
  !Set the problem to be a static/dynamic Navier-Stokes problem
  IF(DYNAMIC_SOLVER_NAVIER_STOKES_FLAG) THEN
    IF(ALE_SOLVER_NAVIER_STOKES_FLAG) THEN
...
!   CALL cmfe_SolverDynamicDynamicSet(DynamicSolverNavierStokes,.TRUE.,Err)
  !Get the dynamic nonlinear solver
    ...
    CALL cmfe_Solver_NewtonJacobianCalculationTypeSet(NonlinearSolverNavierStokes, &
      & CMFE_SOLVER_NEWTON_JACOBIAN_EQUATIONS_CALCULATED, &
      & Err)
    !Set the output type
  ...
  IF(EXPORT_FIELD_IO) THEN
    ...
    CALL cmfe_Fields_NodesExport(Fields,"MasterNavierStokes","FORTRAN",Err)
    CALL cmfe_Fields_ElementsExport(Fields,"MasterNavierStokes","FORTRAN",Err)
...
END PROGRAM NAVIERSTOKESMASTEREXAMPLE
