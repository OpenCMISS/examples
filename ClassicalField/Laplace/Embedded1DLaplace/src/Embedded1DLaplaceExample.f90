...
!> \brief This is an example program to solve a Laplace equation on a 1D
!> cubic Hermite line embedded in 1/2/3 D space using OpenCMISS calls.
!>
!> \section LICENSE
!>
...
PROGRAM Embedded1DLaplaceExample 
  ...
  INTEGER(CMISSIntg) :: numberGlobalXElements,numberOfDimensions
  ...
  WRITE(filename,'(A,"_",I0,"_",I0)') "Embedded1DLaplace",numberOfDimensions,numberGlobalXElements
  ...
  CALL CMISSBasis_QuadratureNumberOfGaussXiSet(basis,[4],err)
  ...
  ELSE
    ...
    CALL CMISSGeneratedMesh_ExtentSet(generatedMesh,[WIDTH,0.0_CMISSDP,0.0_CMISSDP],err)
  ENDIF    
  ...
  IF(numberOfDimensions>=2) CALL CMISSField_ComponentMeshComponentSet(geometricField,CMISS_FIELD_U_VARIABLE_TYPE,2,1,err)
  IF(numberOfDimensions==3) CALL CMISSField_ComponentMeshComponentSet(geometricField,CMISS_FIELD_U_VARIABLE_TYPE,3,1,err)
  ...
  !Make the line curve a bit
  !IF(firstNodeDomain==computationalNodeNumber) THEN
  !  CALL CMISSField_ParameterSetUpdateNode(geometricField,CMISS_FIELD_U_VARIABLE_TYPE,CMISS_FIELD_VALUES_SET_TYPE, &
  !    & 1,CMISS_GLOBAL_DERIV_S1,firstNodeNumber,1,1.0_CMISSDP,err)
  !  IF(numberOfDimensions>=2) CALL CMISSField_ParameterSetUpdateNode(geometricField,CMISS_FIELD_U_VARIABLE_TYPE, &
  !    & CMISS_FIELD_VALUES_SET_TYPE,1,CMISS_GLOBAL_DERIV_S1,firstNodeNumber,2,0.0_CMISSDP,err)
  !  IF(numberOfDimensions==3) CALL CMISSField_ParameterSetUpdateNode(geometricField,CMISS_FIELD_U_VARIABLE_TYPE, &
  !    & CMISS_FIELD_VALUES_SET_TYPE,1,CMISS_GLOBAL_DERIV_S1,firstNodeNumber,3,0.0_CMISSDP,err)
  !ENDIF
  !IF(lastNodeDomain==computationalNodeNumber) THEN
  !  CALL CMISSField_ParameterSetUpdateNode(geometricField,CMISS_FIELD_U_VARIABLE_TYPE,CMISS_FIELD_VALUES_SET_TYPE, &
  !    & 1,CMISS_GLOBAL_DERIV_S1,lastNodeNumber,1,1.0_CMISSDP,err)
  !  IF(numberOfDimensions>=2) CALL CMISSField_ParameterSetUpdateNode(geometricField,CMISS_FIELD_U_VARIABLE_TYPE, &
  !    & CMISS_FIELD_VALUES_SET_TYPE,1,CMISS_GLOBAL_DERIV_S1,lastNodeNumber,2,0.0_CMISSDP,err)
  !  IF(numberOfDimensions==3) CALL CMISSField_ParameterSetUpdateNode(geometricField,CMISS_FIELD_U_VARIABLE_TYPE, &
  !    & CMISS_FIELD_VALUES_SET_TYPE,1,CMISS_GLOBAL_DERIV_S1,lastNodeNumber,3,0.0_CMISSDP,err)
  !ENDIF
  ...
  CALL CMISSFields_NodesExport(fields,"Embedded1DLaplace","FORTRAN",err)
  CALL CMISSFields_ElementsExport(fields,"Embedded1DLaplace","FORTRAN",err)
...
END PROGRAM Embedded1DLaplaceExample
