...
!   INTEGER(CMFEIntg), PARAMETER :: IndependentFieldUserNumberDarcy=10
!   INTEGER(CMFEIntg), PARAMETER :: IndependentFieldUserNumberMatProperties=11
  ...
  INTEGER(CMFEIntg), PARAMETER :: EquationsSetFieldUserNumberDarcy=21
  INTEGER(CMFEIntg), PARAMETER :: EquationsSetFieldUserNumberMatProperties=22

  INTEGER(CMFEIntg), PARAMETER :: SolverMatPropertiesUserNumber=1
  INTEGER(CMFEIntg), PARAMETER :: SolverDarcyUserNumber=2
...
!   INTEGER(CMFEIntg) :: MPI_IERROR
  ...
  INTEGER(CMFEIntg) :: LINEAR_SOLVER_DARCY_OUTPUT_FREQUENCY
  ...
  REAL(CMFEDP) :: INITIAL_FIELD_DARCY(3)
  ...
  REAL(CMFEDP) :: LINEAR_SOLVER_DARCY_START_TIME
  REAL(CMFEDP) :: LINEAR_SOLVER_DARCY_STOP_TIME
  REAL(CMFEDP) :: LINEAR_SOLVER_DARCY_TIME_INCREMENT
...
!   TYPE(cmfe_FieldType) :: IndependentFieldMatProperties
  ...
  INTEGER(CMFEIntg) :: NumberOfComputationalNodes,ComputationalNodeNumber,NodeDomain
  ...
  POROSITY_PARAM_DARCY=0.3_CMFEDP
  ...
  !Set time parameter
  LINEAR_SOLVER_DARCY_START_TIME=0.0_CMFEDP
  LINEAR_SOLVER_DARCY_STOP_TIME=1.125_CMFEDP
  LINEAR_SOLVER_DARCY_TIME_INCREMENT=0.125_CMFEDP
  ...
  RESTART_VALUE=3000_CMFEIntg !default: 30
...
!   CALL cmfe_Equations_LumpingTypeSet(EquationsDarcy,CMFE_EQUATIONS_UNLUMPED_MATRICES,Err)
  ...
  CALL cmfe_ControlLoop_OutputTypeSet(ControlLoop,CMISS_CONTROL_LOOP_PROGRESS_OUTPUT,Err)
    ...
    DO NODE_NUMBER=1_CMFEIntg,NUMBER_OF_NODES_GEOMETRY
      ...
      IF( (ABS(COORD_X-DOMAIN_X1) < GEOMETRY_TOLERANCE) .AND. ComputationalNodeNumber==NodeDomain ) THEN
        ...
        CALL cmfe_BoundaryConditions_SetNode(BoundaryConditionsDarcy,DependentFieldDarcy,CMFE_FIELD_U_VARIABLE_TYPE, &
          & CMFE_NO_GLOBAL_DERIV,1,NODE_NUMBER,1_CMFEIntg,CONDITION,VALUE,Err)
      END IF
      !
      ...
      IF( (ABS(COORD_Y-DOMAIN_Y1) < GEOMETRY_TOLERANCE) .AND. ComputationalNodeNumber==NodeDomain ) THEN
        ...
        VALUE = 2.0_CMFEDP
        CALL cmfe_BoundaryConditions_SetNode(BoundaryConditionsDarcy,DependentFieldDarcy,CMFE_FIELD_U_VARIABLE_TYPE, &
          & CMFE_NO_GLOBAL_DERIV,1,NODE_NUMBER,2_CMFEIntg,CONDITION,VALUE,Err)
      END IF
      !
  ...
  ELSE IF( CM%D==3_CMFEIntg ) THEN
    DO NODE_NUMBER=1_CMFEIntg,NUMBER_OF_NODES_GEOMETRY
      ...
      IF( (ABS(COORD_Z-DOMAIN_Z1) < GEOMETRY_TOLERANCE) .AND. ComputationalNodeNumber==NodeDomain ) THEN
        ...
        VALUE = 1.0_CMFEDP
        CALL cmfe_BoundaryConditions_SetNode(BoundaryConditionsDarcy,DependentFieldDarcy,CMFE_FIELD_U_VARIABLE_TYPE, &
          & CMFE_NO_GLOBAL_DERIV,1,NODE_NUMBER,3_CMFEIntg,CONDITION,VALUE,Err)
      END IF
      !
