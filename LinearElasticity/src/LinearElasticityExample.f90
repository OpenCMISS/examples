...
!###    Index for a partial derivative.
  ...
  IF(USE_GENERATED_MESHES) THEN
    ...
    DO Variable_idx=1,Num_Field_Variables
      CALL FIELD_NUMBER_OF_COMPONENTS_SET(DEPENDENT_FIELD,Variable_idx,Num_Field_Components,ERR,ERROR,*999)
      DO Component_idx=1,Num_Field_Components
        CALL FIELD_COMPONENT_MESH_COMPONENT_SET(DEPENDENT_FIELD,Variable_idx,Component_idx,Mesh_component,ERR,ERROR,*999)
      ENDDO !Component_idx
    ENDDO !Variable_idx
  ...
  DO Variable_idx=1,Num_Field_Variables
    CALL FIELD_NUMBER_OF_COMPONENTS_SET(MATERIAL_FIELD,Variable_idx,Num_Field_Components,ERR,ERROR,*999)
    DO Component_idx=1,Num_Field_Components
  ...
  CALL EQUATIONS_SET_DEPENDENT_CREATE_START(EQUATIONS_SET,4,DEPENDENT_FIELD,ERR,ERROR,*999)
  CALL EQUATIONS_SET_DEPENDENT_CREATE_FINISH(EQUATIONS_SET,ERR,ERROR,*999)

  !Create the equations set material field variables
  CALL EQUATIONS_SET_MATERIALS_CREATE_START(EQUATIONS_SET,2,MATERIAL_FIELD,ERR,ERROR,*999)  
  ...
  DO BC_idx=1,SUM(DISPLACEMENT_BC%NUM_BC_NODES) !Loop over fixed BC nodes
    CALL BOUNDARY_CONDITIONS_SET_NODE(BOUNDARY_CONDITIONS,FIELD_U_VARIABLE_TYPE,Field_Derivative, &
      & DISPLACEMENT_BC%BC_INFO(BC_idx,1),DISPLACEMENT_BC%BC_INFO(BC_idx,2),BOUNDARY_CONDITION_FIXED, &
      & DISPLACEMENT_BC%BC_VALUE(BC_idx),ERR,ERROR,*999)
  ENDDO !BC_idx
