...
!###    Index for a partial derivative.
  ...
  TYPE(BC_SET_TYPE) :: DISPLACEMENT_BC,FORCE_BC
  ...
  INTEGER(INTG) :: User_Num_CS, User_Num_REG, User_Num_BASIS, User_Num_MESH, User_Num_DECOMP,User_Num_FIELD
  ...
  CALL COORDINATE_SYSTEM_TYPE_SET(User_Num_CS,COORDINATE_RECTANGULAR_CARTESIAN_TYPE,ERR,ERROR,*999)
  CALL COORDINATE_SYSTEM_DIMENSION_SET(COORDINATE_SYSTEM,Num_Coordinates,ERR,ERROR,*999)
  CALL COORDINATE_SYSTEM_ORIGIN_SET(User_Num_CS,(/0.0_DP,0.0_DP,0.0_DP/),ERR,ERROR,*999) 
  CALL COORDINATE_SYSTEM_CREATE_FINISH(COORDINATE_SYSTEM,ERR,ERROR,*999)

  !=CREATE REGION==================================================================================================================
  !Create Region and set CS to newly created 3D RC CS
  User_Num_REG = 1
  CALL REGION_CREATE_START(User_Num_REG,REGION,ERR,ERROR,*999)
  ...
  CALL BASIS_TYPE_SET(User_Num_BASIS,BASIS_LAGRANGE_HERMITE_TP_TYPE,ERR,ERROR,*999)
  CALL BASIS_NUMBER_OF_XI_SET(BASIS,Num_xi_coordinates,ERR,ERROR,*999)
  CALL BASIS_INTERPOLATION_XI_SET(User_Num_BASIS,(/BASIS_LINEAR_LAGRANGE_INTERPOLATION, &
    & BASIS_LINEAR_LAGRANGE_INTERPOLATION,BASIS_LINEAR_LAGRANGE_INTERPOLATION/),ERR,ERROR,*999)
  CALL BASIS_QUADRATURE_NUMBER_OF_GAUSS_XI_SET(User_Num_BASIS, &
  ...
  IF(USE_GENERATED_MESHES) THEN
    ...
    User_Num_FIELD = 1  
    CALL FIELD_CREATE_START(User_Num_FIELD,REGION,GEOMETRIC_FIELD,ERR,ERROR,*999)
    ...
    DO Component_idx=1,Num_Field_Components 
      CALL FIELD_COMPONENT_MESH_COMPONENT_SET(User_Num_FIELD,Field_Variable,Component_idx,REGION,Mesh_component,ERR,ERROR,*999)
    ENDDO !Component_idx
    ...
    Num_Field_Components = 3
    CALL FIELD_CREATE_START(User_Num_FIELD,REGION,DEPENDENT_FIELD,ERR,ERROR,*999)
    CALL FIELD_TYPE_SET(User_Num_FIELD,REGION,FIELD_GENERAL_TYPE,ERR,ERROR,*999)  
    CALL FIELD_MESH_DECOMPOSITION_SET(DEPENDENT_FIELD,DECOMPOSITION,ERR,ERROR,*999)
    CALL FIELD_GEOMETRIC_FIELD_SET(User_Num_FIELD,REGION,GEOMETRIC_FIELD,ERR,ERROR,*999) 
    CALL FIELD_DEPENDENT_TYPE_SET(User_Num_FIELD,REGION,FIELD_DEPENDENT_TYPE,ERR,ERROR,*999) 
    CALL FIELD_NUMBER_OF_VARIABLES_SET(User_Num_FIELD,REGION,Num_Field_Variables,ERR,ERROR,*999)
    CALL FIELD_NUMBER_OF_COMPONENTS_SET(User_Num_FIELD,REGION,Num_Field_Components,ERR,ERROR,*999)
    DO Variable_idx=1,Num_Field_Variables
      DO Component_idx=1,Num_Field_Components
        CALL FIELD_COMPONENT_MESH_COMPONENT_SET(User_Num_FIELD,Variable_idx,Component_idx,REGION,Mesh_component,ERR,ERROR,*999)
      ENDDO !Component_idx
    ENDDO !Variable_idx
  ...
  ELSE
    ...
    Num_Nodes = 8
    CALL NODES_CREATE_START(Num_Nodes,REGION,NODES,ERR,ERROR,*999)
    DO np=1,Num_Nodes
      CALL NODE_INITIAL_POSITION_SET(np,(/0.0_DP,0.0_DP,0.0_DP/),NODES,ERR,ERROR,*999)
    ENDDO !np
    ...
    CALL MESH_NUMBER_OF_COMPONENTS_SET(User_Num_Mesh,REGION,Num_mesh_components,ERR,ERROR,*999) 
    ...
    !Set the decomposition to use
    CALL FIELD_MESH_DECOMPOSITION_SET(GEOMETRIC_FIELD,DECOMPOSITION,ERR,ERROR,*999)
    CALL FIELD_TYPE_SET(User_Num_FIELD,REGION,FIELD_GEOMETRIC_TYPE,ERR,ERROR,*999)  
    CALL FIELD_NUMBER_OF_VARIABLES_SET(User_Num_FIELD,REGION,Num_Field_Variables,ERR,ERROR,*999)
    CALL FIELD_NUMBER_OF_COMPONENTS_SET(User_Num_FIELD,REGION,Num_Field_Components,ERR,ERROR,*999) 
    !Set the domain to be used by the field components
    !NB these are needed now as the default mesh component number is 1
    ...
    DO np=1,Num_Nodes
      DO Component_idx=1,Num_Field_Components
      CALL FIELD_PARAMETER_SET_UPDATE_NODE(GEOMETRIC_FIELD,FIELD_VALUES_SET_TYPE,Field_Derivative,np,Component_idx, &
        & Field_Variable,Node_Coordinates(np,Component_idx),ERR,ERROR,*999)
      ENDDO !Component_idx
    ENDDO !np

  !NOT REQUIRED TO CREATE A DEPENDENT FIELD
  ...
  CALL FIELD_CREATE_START(User_Num_FIELD,REGION,MATERIAL_FIELD,ERR,ERROR,*999)
  CALL FIELD_TYPE_SET(User_Num_FIELD,REGION,FIELD_MATERIAL_TYPE,ERR,ERROR,*999)
  ...
  DO np=1,Num_nodes
    DO Component_idx=1,Num_Field_Components
      CALL FIELD_PARAMETER_SET_UPDATE_NODE(MATERIAL_FIELD,FIELD_VALUES_SET_TYPE,Field_Derivative,np,Component_idx,1, &
	     & E_v(Component_idx),ERR,ERROR,*999)
    ENDDO !Component_idx
  ENDDO !np
  ...
  DISPLACEMENT_BC%NUM_BC_NODES = (/4,4,4/)
  IF (SUM(DISPLACEMENT_BC%NUM_BC_NODES) == 0) THEN
    CALL FLAG_ERROR("No Displacement Boundary Conditions Applied.",ERR,ERROR,*999)
  ENDIF
  ALLOCATE(DISPLACEMENT_BC%BC_INFO(SUM(DISPLACEMENT_BC%NUM_BC_NODES),2),STAT=ERR)
  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate BC_NODE.",ERR,ERROR,*999)
  ALLOCATE(DISPLACEMENT_BC%BC_VALUE(SUM(DISPLACEMENT_BC%NUM_BC_NODES)),STAT=ERR)
  IF(ERR/=0) CALL FLAG_ERROR("Could not allocate BC_VALUE.",ERR,ERROR,*999)
  IF (DISPLACEMENT_BC%NUM_BC_NODES(1) > 0) THEN 
    DISPLACEMENT_BC%BC_INFO(1:DISPLACEMENT_BC%NUM_BC_NODES(1),1) = (/1,3,5,7/)
    DISPLACEMENT_BC%BC_INFO(1:DISPLACEMENT_BC%NUM_BC_NODES(1),2) = 1
    DISPLACEMENT_BC%BC_VALUE(1:DISPLACEMENT_BC%NUM_BC_NODES(1)) = 0.0_DP
  ENDIF
  IF (DISPLACEMENT_BC%NUM_BC_NODES(2) > 0) THEN 
    DISPLACEMENT_BC%BC_INFO(DISPLACEMENT_BC%NUM_BC_NODES(1)+1:SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:2)),1) = (/1,3,5,7/)
    DISPLACEMENT_BC%BC_INFO(DISPLACEMENT_BC%NUM_BC_NODES(1)+1:SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:2)),2) = 2
    DISPLACEMENT_BC%BC_VALUE(DISPLACEMENT_BC%NUM_BC_NODES(1)+1:SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:2))) = 0.0_DP
  ENDIF
  IF (DISPLACEMENT_BC%NUM_BC_NODES(3) > 0) THEN 
    DISPLACEMENT_BC%BC_INFO(SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:2))+1:SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:3)),1) = (/1,3,5,7/)
    DISPLACEMENT_BC%BC_INFO(SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:2))+1:SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:3)),2) = 3
    DISPLACEMENT_BC%BC_VALUE(SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:2))+1:SUM(DISPLACEMENT_BC%NUM_BC_NODES(1:3))) = 0.0_DP
  ENDIF
  ...
  DO BC_idx=1,SUM(DISPLACEMENT_BC%NUM_BC_NODES) !Loop over fixed BC nodes
    CALL EQUATIONS_SET_FIXED_CONDITIONS_SET_NODE(EQUATIONS_SET,FIELD_BOUNDARY_CONDITIONS_SET_TYPE, &
      & Field_Derivative,DISPLACEMENT_BC%BC_INFO(BC_idx,1),DISPLACEMENT_BC%BC_INFO(BC_idx,2),Field_Variable, &
      & EQUATIONS_SET_FIXED_BOUNDARY_CONDITION,DISPLACEMENT_BC%BC_VALUE(BC_idx),ERR,ERROR,*999)
  ENDDO !BC_idx
  ...
  IF (FORCE_BC%NUM_BC_NODES(1) > 0) THEN 
    FORCE_BC%BC_INFO(1:FORCE_BC%NUM_BC_NODES(1),1) = (/2,4,6,8/)
  ...
  !Update Equation set geometric, dependent and material fields 
  ...
  CALL PROBLEM_SOLVER_EQUATIONS_EQUATIONS_SET_ADD(PROBLEM,CONTROL_LOOP_NODE,Solver_Idx,EQUATIONS_SET,Equations_Set_Idx,ERR,ERROR,*999)
