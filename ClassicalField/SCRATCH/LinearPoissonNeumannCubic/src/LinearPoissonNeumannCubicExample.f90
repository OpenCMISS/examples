...
!  CALL CMISSDiagnosticsSetOn(CMISSInDiagType,DIAG_LEVEL_LIST,"LinearPoissonDiagnostics",DIAG_ROUTINE_LIST,Err)
  ...
  IF(NUMBER_GLOBAL_Z_ELEMENTS==0) THEN
    ...
    CALL CMISSBasisInterpolationXiSet(Basis,(/3,3/),Err) !Cubic, linear is default
    CALL CMISSBasisQuadratureNumberOfGaussXiSet(Basis,(/4,4/),Err) !3 Gauss per Xi direction per element
  ELSE
  ...
  CALL CMISSEquationsSetAnalyticCreateStart(EquationsSet,CMISSTestCaseMixedNeumannDirichlet_3, &
