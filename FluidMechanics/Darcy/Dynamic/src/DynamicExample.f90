...
!   INTEGER(CMFEIntg) :: MPI_IERROR
  ...
  PERM_OVER_VIS_PARAM_DARCY=0.8_CMFEDP 
  ...
  DYNAMIC_SOLVER_DARCY_STOP_TIME=5.0_CMFEDP 
  DYNAMIC_SOLVER_DARCY_TIME_INCREMENT=1.0_CMFEDP
...
!   CALL cmfe_DiagnosticsSetOn(CMFE_IN_DIAG_TYPE,DIAG_LEVEL_LIST,"DarcyDiagnostics",DIAG_ROUTINE_LIST,Err)
  ...
  DO NODE_NUMBER=1,NUMBER_OF_NODES_GEOMETRY
    DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
      CALL cmfe_Decomposition_NodeDomainGet(Decomposition,NODE_NUMBER,MESH_COMPONENT_NUMBER_GEOMETRY,NodeDomain,Err)
  ...
  ELSE IF( CM%D==3_CMFEIntg ) THEN
    DO NODE_NUMBER=1_CMFEIntg,NUMBER_OF_NODES_GEOMETRY
      ...
      IF( (ABS(COORD_Z-DOMAIN_Z1) < GEOMETRY_TOLERANCE) .AND. ComputationalNodeNumber==NodeDomain ) THEN
        ...
        CALL cmfe_BoundaryConditions_SetNode(BoundaryConditionsDarcy,DependentFieldDarcy,CMFE_FIELD_U_VARIABLE_TYPE, &
          & CMFE_NO_GLOBAL_DERIV,1, &
          & NODE_NUMBER,3_CMFEIntg,CONDITION,VALUE,Err)
      END IF
      !
