...
PROGRAM DIFFUSIONEXAMPLE
  ...
  TYPE(cmfe_FieldType) :: GeometricField,DependentField,MaterialsField,AnalyticField,EquationsSetField
...
!   CALL cmfe_EquationsSet_SpecificationSet(EquationsSet,CMFE_EQUATIONS_SET_CLASSICAL_FIELD_CLASS, &
!     & CMFE_EQUATIONS_SET_DIFFUSION_EQUATION_TYPE,CMFE_EQUATIONS_SET_NO_SOURCE_DIFFUSION_SUBTYPE,Err)
...
!   CALL EQUATIONS_SET_BOUNDARY_CONDITIONS_CREATE_FINISH(EQUATIONS_SET,ERR,ERROR,*999)
  ...
  !Set the times
  CALL cmfe_ControlLoop_TimesSet(ControlLoop,0.0_CMFEDP,1.001_CMFEDP,0.001_CMFEDP,Err)
...
!   CALL PROBLEM_SOLVERS_CREATE_FINISH(PROBLEM,ERR,ERROR,*999)
  ...
  Call cmfe_AnalyticAnalysisOutput(DependentField,"DiffusionAnalytics_x40_y40_L_T1",Err)
  ...
  IF(EXPORT_FIELD) THEN
    ...
    CALL cmfe_Fields_NodesExport(Fields,"Diffusion_x40_y40_L_T1","FORTRAN",Err)
    CALL cmfe_Fields_ElementsExport(Fields,"Diffusion_x40_y40_L_T1","FORTRAN",Err)
