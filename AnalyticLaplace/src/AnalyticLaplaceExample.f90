...
PROGRAM ANALYTICLAPLACEEXAMPLE
  ...
  REAL(DP), PARAMETER :: HEIGHT=3.0_DP
  REAL(DP), PARAMETER :: WIDTH=3.0_DP
  ...
  INTEGER(INTG) :: NUMBER_OF_NODES, node_idx
  REAL(DP) :: node_x, node_y, node_z, value
  ...
  ELSE
    ...
    CALL FIELD_GEOMETRIC_PARAMETERS_UPDATE_FROM_INITIAL_MESH(GEOMETRIC_FIELD,MESH,ERR,ERROR,*999)
...
!    & 1.0_DP,ERR,ERROR,*999)
  ...
  ! Set bc's Some hack to create the boundary conditions
  IF(NUMBER_GLOBAL_Z_ELEMENTS/=0) THEN
    NUMBER_OF_NODES=(NUMBER_GLOBAL_X_ELEMENTS+1)*(NUMBER_GLOBAL_Y_ELEMENTS+1)*(NUMBER_GLOBAL_Z_ELEMENTS+1)
    DO node_idx=1,NUMBER_OF_NODES
      node_x=MOD(node_idx-1,NUMBER_GLOBAL_X_ELEMENTS+1)*WIDTH/NUMBER_GLOBAL_X_ELEMENTS
      node_y=MOD(node_idx-1,(NUMBER_GLOBAL_X_ELEMENTS+1)*(NUMBER_GLOBAL_Y_ELEMENTS+1))/(NUMBER_GLOBAL_X_ELEMENTS+1)*WIDTH/NUMBER_GLOBAL_Y_ELEMENTS
      node_z=(node_idx-1)/((NUMBER_GLOBAL_X_ELEMENTS+1)*(NUMBER_GLOBAL_Y_ELEMENTS+1))*WIDTH/NUMBER_GLOBAL_Z_ELEMENTS
      IF (node_x==0.0_DP.OR.node_x==WIDTH.OR.node_y==0.0_DP.OR.node_y==HEIGHT.OR.node_z==0.0_DP.OR.node_z==LENGTH) THEN 
        value=node_x**2-2*node_y**2+node_z**2
        CALL EQUATIONS_SET_FIXED_CONDITIONS_SET_NODE(EQUATIONS_SET,FIELD_BOUNDARY_CONDITIONS_SET_TYPE,1,node_idx,1,FIELD_STANDARD_VARIABLE_TYPE,EQUATIONS_SET_FIXED_BOUNDARY_CONDITION,value,ERR,ERROR,*999)
      ENDIF
    ENDDO
  ELSE
    NUMBER_OF_NODES=(NUMBER_GLOBAL_X_ELEMENTS+1)*(NUMBER_GLOBAL_Y_ELEMENTS+1)
    DO node_idx=1,NUMBER_OF_NODES
    IF(node_idx<=NUMBER_GLOBAL_X_ELEMENTS.OR.node_idx>(NUMBER_OF_NODES-NUMBER_GLOBAL_X_ELEMENTS-1) &
      & .OR.MOD(node_idx,NUMBER_GLOBAL_X_ELEMENTS+1)==1.OR.MOD(node_idx,NUMBER_GLOBAL_X_ELEMENTS+1)==0) THEN
      node_x=MOD(node_idx-1,NUMBER_GLOBAL_X_ELEMENTS+1)*WIDTH/NUMBER_GLOBAL_X_ELEMENTS
      node_y=(node_idx-1)/(NUMBER_GLOBAL_X_ELEMENTS+1)*WIDTH/NUMBER_GLOBAL_Y_ELEMENTS
      value=node_x**2+2*node_x*node_y-node_y**2
      CALL EQUATIONS_SET_FIXED_CONDITIONS_SET_NODE(EQUATIONS_SET,FIELD_BOUNDARY_CONDITIONS_SET_TYPE,1,node_idx,1,FIELD_STANDARD_VARIABLE_TYPE,EQUATIONS_SET_FIXED_BOUNDARY_CONDITION,value,ERR,ERROR,*999)
    ENDIF
  ENDDO
  ENDIF
  ...
  IF(NUMBER_GLOBAL_Z_ELEMENTS/=0) THEN
    CALL EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET(EQUATIONS_SET,LAPLACE_EQUATION_THREE_DIM_1,ERR,ERROR,*999)
  ELSE
    CALL EQUATIONS_SET_ANAYLITIC_EQUATION_NUMBER_SET(EQUATIONS_SET,LAPLACE_EQUATION_TWO_DIM_1,ERR,ERROR,*999)
  ENDIF
  !Finish the equations set dependent field variables
