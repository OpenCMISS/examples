...
PROGRAM NAVIERSTOKESDYNAMICEXAMPLE
  ...
  INTEGER(CMFEIntg), PARAMETER :: EquationsSetUserNumberNavierStokes=8
...
!   INTEGER(CMFEIntg) :: MPI_IERROR
  ...
  DYNAMIC_SOLVER_NAVIER_STOKES_STOP_TIME=1.00010_CMFEDP 
  ...
  CALL cmfe_Decomposition_ElementDomainSet(Decomposition,3,0,Err)
  CALL cmfe_Decomposition_ElementDomainSet(Decomposition,4,1,Err)
  CALL cmfe_Decomposition_ElementDomainSet(Decomposition,5,1,Err)
  CALL cmfe_Decomposition_ElementDomainSet(Decomposition,6,1,Err)
...
!   CALL cmfe_Solver_NewtonJacobianCalculationTypeSet(NonlinearSolverNavierStokes,CMFE_SOLVER_NEWTON_JACOBIAN_EQUATIONS_CALCULATED,Err)
  !Set the output type
