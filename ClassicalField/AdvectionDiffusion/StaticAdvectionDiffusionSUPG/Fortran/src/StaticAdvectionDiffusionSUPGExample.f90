...
#include "mpif.h"
  ...
  CALL cmfe_EquationsSet_CreateStart(EquationsSetUserNumber,Region,GeometricField,[CMFE_EQUATIONS_SET_CLASSICAL_FIELD_CLASS, &
    & CMFE_EQUATIONS_SET_ADVECTION_DIFFUSION_EQUATION_TYPE,CMFE_EQUATIONS_SET_CONST_SOURCE_STATIC_ADVEC_DIFF_SUPG_SUBTYPE], &
...
! 
 ...
 !Output Analytic analysis
  Call cmfe_AnalyticAnalysis_Output(DependentField,"StaticAdvectionDiffusionSUPGAnalytics",Err)

  EXPORT_FIELD=.TRUE.
  IF(EXPORT_FIELD) THEN
    CALL cmfe_Fields_Initialise(Fields,Err)
    CALL cmfe_Fields_Create(Region,Fields,Err)
    CALL cmfe_Fields_NodesExport(Fields,"StaticAdvectionDiffusionSUPG","FORTRAN",Err)
    CALL cmfe_Fields_ElementsExport(Fields,"StaticAdvectionDiffusionSUPG","FORTRAN",Err)
    CALL cmfe_Fields_Finalise(Fields,Err)
