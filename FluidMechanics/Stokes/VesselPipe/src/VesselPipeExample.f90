...
!> \example FluidMechanics/Stokes/VesselPipe/src/VesselPipeExample.f90
!! Example program to solve a Stokes equation using openCMISS calls.
!! \par Latest Builds:
!! \li <a href='http://autotest.bioeng.auckland.ac.nz/opencmiss-build/logs_x86_64-linux/FluidMechanics/Stokes/VesselPipe/build-intel'>Linux Intel Build</a>
!! \li <a href='http://autotest.bioeng.auckland.ac.nz/opencmiss-build/logs_x86_64-linux/FluidMechanics/Stokes/VesselPipe/build-gnu'>Linux GNU Build</a>
!<
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  CALL BASIS_CREATE_START(CM%ID_M,BASIS_M,ERR,ERROR,*999)
      ...
      ELSE IF (CM%D==3) THEN
        ...
        CALL BASIS_QUADRATURE_NUMBER_OF_GAUSS_XI_SET(BASIS_M,(/4,4,4/),ERR,ERROR,*999)
      ELSE
        GOTO 999
      END IF
  ...
  CALL BASIS_CREATE_START(CM%ID_V,BASIS_V,ERR,ERROR,*999)
      ...
      ELSE IF (CM%D==3) THEN
        ...
        CALL BASIS_QUADRATURE_NUMBER_OF_GAUSS_XI_SET(BASIS_V,(/4,4,4/),ERR,ERROR,*999)
      ELSE
        GOTO 999
      END IF
  ...
  CALL BASIS_CREATE_START(CM%ID_P,BASIS_P,ERR,ERROR,*999)
      ...
      ELSE IF (CM%D==3) THEN   !User variables
        CALL BASIS_INTERPOLATION_XI_SET(BASIS_P,(/CM%IT_P,CM%IT_P,CM%IT_P/),ERR,ERROR,*999)
        CALL BASIS_QUADRATURE_NUMBER_OF_GAUSS_XI_SET(BASIS_P,(/4,4,4/),ERR,ERROR,*999)
      ELSE
        GOTO 999
      END IF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   BC_WALL_NODES=(/65,66,67,68,77,78,87,88,89,90,99,100,101,102,111,112,113,114,121,122,&
   &129,130,131,138,139,140,147,148,149,150,157,158,165,166,167,174,175,176,183,184,189,&
   &190,195,196,201,202,207,208,257,258,267,268,269,270,279,280,281,282,291,292,293,300,&
   &301,302,309,310,311,318,319,320,327,328,329,336,337,338,345,346,351,352,357,358,363,&
   &364,413,414,423,424,425,426,435,436,437,438,447,448,449,456,457,458,465,466,467,474,&
   &475,476,483,484,485,492,493,494,501,502,507,508,513,514,519,520,569,570,579,580,581,&
   &582,591,592,593,594,603,604,605,612,613,614,621,622,623,630,631,632,639,640,641,648,&
   &649,650,657,658,663,664,669,670,675,676,735,736,737,738,747,748,749,750,768,769,770,&
   &725,726,759,760,761,786,787,788,813,814,831,832,&
   &777,778,779,795,796,797,804,805,806,819,820,825,826/)
  ...
  &751,752,753,754,755,756,757,758,780,781,782,783,784,785,807,&
  &808,809,810,811,812,827,828,829,830/)
