...
%solving using strang splitting techniques to test code against opencmiss code.
...
%ODE SOLVE 1
        %First solve the ode part for tstep<timestep<tstep+dt/2
        initT = t(tstep);
        halfT = t(tstep)+(dt*0.5);
        finalT = t(tstep)+dt;

	time_interval = halfT-initT;
	%evaluate the ode at each of the spatially discretized nodes.
	tintvl = [initT:odestep:halfT];
	uintmd_one = zeros(numNod,size(tintvl,2));
	uintmd_one(1,:) = 1.5;
	uintmd_one(numNod,:) = 1.5; 
	for node = 2:numNod-1
...
%ODE SOLVE
    %now solve ode part again from initT to halfT with initial u values from the pde solve.
	time_interval = finalT-halfT;
	tintvl = [halfT:odestep:finalT];
	uintmd_three = zeros(numNod,size(tintvl,2));
	%evaluate the ode at each of the spatially discretized nodes.
	uintmd_three(1,:) = 1.5;
	uintmd_three(numNod,:) = 1.5;
	for node = 2:numNod-1
		uintmd_three(node,:) = lsode("constfunc",uintmd_two(node),tintvl);
	end

	%the final u value for the overall pde is the value from this last ode solver at dt/2
	u(:,tstep+1) = uintmd_three(1:numNod,size(uintmd_three,2));
end		
u(:,size(u,2))
