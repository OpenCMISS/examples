...
! ! 
! !  This example considers permeability to be anisotropic or
! !  transversely isotropic and, thus, no longer to be a scalar
! !  but a material tensor.
! ! 
...
!     & FIELD_VALUES_SET_TYPE,1,DARCY%PERM_OVER_VIS,ERR,ERROR,*999)
  ...
  k_xx = 1.0_DP * DARCY%PERM_OVER_VIS

  CALL FIELD_COMPONENT_VALUES_INITIALISE(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE,&
    & FIELD_VALUES_SET_TYPE,1,k_xx,ERR,ERROR,*999)

  k_yy = 1.0_DP * DARCY%PERM_OVER_VIS

  CALL FIELD_COMPONENT_VALUES_INITIALISE(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE,&
    & FIELD_VALUES_SET_TYPE,2,k_yy,ERR,ERROR,*999)

!   k_zz = 3.0_DP * DARCY%PERM_OVER_VIS
! 
!   CALL FIELD_COMPONENT_VALUES_INITIALISE(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE,&
!     & FIELD_VALUES_SET_TYPE,3,k_zz,ERR,ERROR,*999)
...
!     CALL EQUATIONS_OUTPUT_TYPE_SET(EQUATIONS,EQUATIONS_ELEMENT_MATRIX_OUTPUT,ERR,ERROR,*999)
  ...
  IF( CM%D==2 ) THEN
    ...
    DO j=1,DARCY%BC_NUMBER_OF_WALL_NODES
      ...
      IF( (ABS(COORD_X-DARCY%X1) < DARCY%GEOM_TOL) ) THEN
        !x-velocity
        dummy = dummy + 1
        i = 1
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
        DOF_VALUES( dummy ) = 1.0_DP
      END IF
      !
      IF( (ABS(COORD_X-DARCY%X2) < DARCY%GEOM_TOL) ) THEN
        !x-velocity
        dummy = dummy + 1
        i = 1
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
        DOF_VALUES( dummy ) = 3.0_DP
      END IF
      !
      IF( (ABS(COORD_Y-DARCY%Y1) < DARCY%GEOM_TOL) ) THEN
        !y-velocity
        dummy = dummy + 1
        i = 2
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
        DOF_VALUES( dummy ) = 2.0_DP
      END IF
      !
      IF( (ABS(COORD_Y-DARCY%Y2) < DARCY%GEOM_TOL) ) THEN
        !y-velocity
        dummy = dummy + 1
        i = 2
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
  ...
  ELSE IF( CM%D==3 ) THEN
    ...
    DO j=1,DARCY%BC_NUMBER_OF_WALL_NODES
      ...
      END IF
      !
      IF( (ABS(COORD_Z-DARCY%Z1) < DARCY%GEOM_TOL) ) THEN
        !z-velocity
        dummy = dummy + 1
        i = 3
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
      ...
      END IF
      !
      IF( (ABS(COORD_Z-DARCY%Z2) < DARCY%GEOM_TOL) ) THEN
        !z-velocity
        dummy = dummy + 1
        i = 3
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
