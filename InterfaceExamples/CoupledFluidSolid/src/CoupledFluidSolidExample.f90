...
PROGRAM CoupledFluidSolidExample
  ...
  REAL(CMFEDP) :: zeroCheck=1.0_CMFEDP
  ...
  INTEGER(CMFEIntg), ALLOCATABLE :: NodeIndices(:),NodeIndexFMappings(:),NodeIndexSMappings(:), &
    & SolidElements(:), &
  ...
  REAL(CMFEDP), ALLOCATABLE :: SolidGeometry(:),FluidGeometry(:),InterfaceGeometry(:),InterfaceXiPosition(:), &
  ...
  INTEGER(CMFEIntg), ALLOCATABLE :: IndexArray(:)
  ...
  INTEGER(CMFEIntg) :: NUMBER_OF_ARGUMENTS,STATUS
  INTEGER(CMFEIntg) :: NumberGlobalElementsX, &
  ...
  INTEGER(CMFEIntg) :: MMCondition=0
  ...
  ! LOOP INTEGERS
  INTEGER(CMFEIntg) :: NodeNumber,S,P,ElementIndex,NodeIndex,MM,MaterialSpecification,LocalNodeIndex
  ...
  TYPE(cmfe_BasisType) :: BasisSpaceFluid,BasisVelocity,BasisPressure
  TYPE(cmfe_BasisType) :: InterfaceBasis1,InterfaceMappingBasis1
  
  TYPE(cmfe_NodesType) :: SolidNodes
  TYPE(cmfe_NodesType) :: FluidNodes,InterfaceNodes
  TYPE(cmfe_MeshElementsType) :: SolidMeshElementsSpace,SolidMeshElementsDisplacement,SolidMeshElementsHydrostaticPressure
  TYPE(cmfe_MeshElementsType) :: FluidMeshElementsSpace,FluidMeshElementsVelocity,FluidMeshElementsPressure
  ...
  TYPE(cmfe_FieldType) :: MaterialField1,MaterialField2,EquationsSetFieldMovingMesh,SourceField1
  ...
  TYPE(cmfe_SolverType) :: DynamicSolver
  TYPE(cmfe_SolverType) :: NonlinearSolver
  ...
  LinearSolverMovingMesh_OutputType=CMFE_SOLVER_SOLVER_OUTPUT
  DynamicSolver_OutputType=CMFE_SOLVER_SOLVER_OUTPUT
  LinearSolver_OutputType=CMFE_SOLVER_SOLVER_OUTPUT
  NonlinearSolver_OutputType=CMFE_SOLVER_SOLVER_OUTPUT
  ...
  CASE(Plate2D)
    ...
    Gravity(:)=[0.0_CMFEDP,9.81_CMFEDP] !in m s^-2
  CASE(Plate3D)
    ...
    IF(CheckWithoutInterfaceCondition) THEN
      GravityFlag=.TRUE.
      ALLOCATE(Gravity(NumberOfDimensions))
      Gravity(:)=[0.0_CMFEDP,9.81_CMFEDP,0.0_CMFEDP] !in m s^-2
    ENDIF
  END SELECT
...
!  CALL cmfe_Field_CreateFinish(FibreField,Err)
  ...
  CALL cmfe_Problem_SolverGet(CoupledProblem,CMFE_CONTROL_LOOP_NODE,DynamicSolverIndex, &
    & DynamicSolver,Err)
  ...
  CALL cmfe_Solver_NewtonJacobianCalculationTypeSet(NonlinearSolver,CMFE_SOLVER_NEWTON_JACOBIAN_EQUATIONS_CALCULATED, &
    & Err)
  ...
  CALL cmfe_SolverEquations_EquationsSetAdd(LinearSolverMovingMeshEquations,MovingMeshEquationsSet, &
  ...
  CALL cmfe_InterfaceMatrices_TimeDependenceTypeSet(InterfaceCondition, &
