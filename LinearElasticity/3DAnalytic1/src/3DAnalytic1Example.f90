...
!> \brief This example illustrates the use of openCMISS to solve an isotropic 3D Linear Elasticity uniaxial extension problem and check with its Analytic Solution.
!>
!> \section LICENSE
!>
...
!> \example LinearElasticity/3DAnalytic1/src/3DAnalytic1Example.f90
!! This example illustrates the use of openCMISS to solve an isotropic 3D Linear Elasticity uniaxial extension problem and check with its Analytic Solution.
!<
...
999 CALL ERRORS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_CONVERGENCE",ERR,ERROR)
  ...
  SUBROUTINE ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT(NUMBER_GLOBAL_X_ELEMENTS,NUMBER_GLOBAL_Y_ELEMENTS, &
    ...
    CALL ANALYTIC_ANALYSIS_OUTPUT(FIELD,"3DANALYTIC_LINEAR_ELASTICITY_"//TESTCASE_INTERPOLATION_SPECIFICATION,ERR,ERROR,*999)
...
999 CALL ERRORS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT",ERR,ERROR)
  ...
  !  & ERR,ERROR,*)
    ...
    TYPE(FIELD_TYPE), POINTER :: GEOMETRIC_FIELD,MATERIAL_FIELD,ANALYTIC_FIELD
    ...
    DO ne=1,NUMBER_OF_ELEMENTS
      DO xi=1,NUMBER_OF_XI
        MESH_COMP_IDX=xi
        ALLOCATE(ELEMENTS(ne)%MESH_COMP(MESH_COMP_IDX)%ELEM_NODE_NUMBERS(MESH_COMP_NUMBER_OF_ELEMENT_NODES(MESH_COMP_IDX)),&
          & STAT=ERR)
    ...
    MATE_PARA = (/30E6_DP,30E6_DP,30E6_DP,0.25_DP,0.25_DP,0.25_DP/)
    !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
