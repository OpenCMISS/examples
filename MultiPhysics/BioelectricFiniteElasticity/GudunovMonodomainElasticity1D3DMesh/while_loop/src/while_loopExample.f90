...
!  REAL(CMISSRP), PARAMETER :: STIM_STOP=0.01_CMISSRP!ELASTICITY_TIME_STEP
  ...
  REAL(CMISSRP), PARAMETER :: Vmax=-0.02_CMISSRP ! 0.02 cm/ms = 0.2 m/s, rat GM    ! this is how it was!!!
...
!  REAL(CMISSRP) :: YVALUE
  ...
  TYPE(cmfe_ControlLoopType) :: ControlLoopMain,ControlLoopM,ControlLoopFE
  ...
  TYPE(cmfe_SolverType) :: SolverDAE,SolverParabolic,SolverFE,LinearSolverFE
  TYPE(cmfe_SolverEquationsType) :: SolverEquationsM,SolverEquationsFE
  TYPE(cmfe_NodesType) :: Nodes
  TYPE(cmfe_MeshElementsType) :: QuadraticElements,LinearElements,ElementsM
...
!   & Vmax/(NumberOfInSeriesFibres*NumberOfNodesPerFibre-1),Err)
  ...
  CALL cmfe_Field_NumberOfComponentsSet(MaterialFieldM,CMFE_FIELD_U_VARIABLE_TYPE,3,Err)
...
!  independent_field_auto_create = .TRUE.
  ...
  CALL cmfe_Field_ComponentValuesInitialise(IndependentFieldFE,CMFE_FIELD_U1_VARIABLE_TYPE,CMFE_FIELD_PREVIOUS_VALUES_SET_TYPE, &
   & 1,1.0_CMISSRP,Err) !fibre stretch of previous step
...
!  CALL cmfe_Field_ParameterSetUpdateFinish(CellMLModelsField,CMFE_FIELD_U_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,Err)
  ...
  CALL cmfe_Problem_CreateStart(ProblemUserNumber,[CMFE_PROBLEM_MULTI_PHYSICS_CLASS, &
    & CMFE_PROBLEM_BIOELECTRIC_FINITE_ELASTICITY_TYPE,CMFE_PROBLEM_MONODOMAIN_ELASTICITY_VELOCITY_SUBTYPE],Problem,Err)
  ...
  CALL cmfe_Solver_NewtonMaximumIterationsSet(SolverFE,1000,Err)
...
!        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.05_CMISSRP,Err)
        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.0_CMISSRP,Err)
    ENDIF
  ENDDO
...
!!!      CALL cmfe_BoundaryConditions_SetNode(BoundaryConditionsFE,DependentFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,1,1,NodeNumber,1, &
!!!!        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.2_CMISSRP,Err)
!!!        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.1_CMISSRP,Err)
!!!    ENDIF
!!!  ENDDO
...
!!!      CALL cmfe_BoundaryConditions_SetNode(BoundaryConditionsFE,DependentFieldFE,CMFE_FIELD_U_VARIABLE_TYPE,1,1,NodeNumber,1, &
!!!!        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.2_CMISSRP,Err)
!!!        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.15_CMISSRP,Err)
!!!    ENDIF
!!!  ENDDO
