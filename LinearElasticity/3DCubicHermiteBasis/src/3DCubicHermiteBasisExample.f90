...
!###    Index for a partial derivative.
  ...
  XI_INTERPOLATION(1,:) = &
    & (/BASIS_CUBIC_HERMITE_INTERPOLATION,BASIS_CUBIC_HERMITE_INTERPOLATION,BASIS_CUBIC_HERMITE_INTERPOLATION/)
  XI_INTERPOLATION(2,:) = &
    & (/BASIS_CUBIC_HERMITE_INTERPOLATION,BASIS_CUBIC_HERMITE_INTERPOLATION,BASIS_CUBIC_HERMITE_INTERPOLATION/)
  XI_INTERPOLATION(3,:) = &
    & (/BASIS_CUBIC_HERMITE_INTERPOLATION,BASIS_CUBIC_HERMITE_INTERPOLATION,BASIS_CUBIC_HERMITE_INTERPOLATION/)
  ...
  ELEMENT(1)%MESH_COMP(1)%ELEM_NODE_NUMBERS(:) = (/1,2,3,4,5,6,7,8/)
  ELEMENT(1)%MESH_COMP(2)%ELEM_NODE_NUMBERS(:) = (/1,2,3,4,5,6,7,8/)
  ELEMENT(1)%MESH_COMP(3)%ELEM_NODE_NUMBERS(:) = (/1,2,3,4,5,6,7,8/)
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  ELEMENT(1)%MESH_COMP(1)%DERIV(1)%NODE_COORDINATES = (/0.0_DP,l,0.0_DP,l,0.0_DP,l,0.0_DP,l/)
  ELEMENT(1)%MESH_COMP(1)%DERIV(2)%NODE_COORDINATES = 1.0_DP
  ELEMENT(1)%MESH_COMP(2)%DERIV(1)%NODE_COORDINATES = (/0.0_DP,0.0_DP,w,w,0.0_DP,0.0_DP,w,w/)
  ELEMENT(1)%MESH_COMP(2)%DERIV(3)%NODE_COORDINATES = 1.0_DP
  ELEMENT(1)%MESH_COMP(3)%DERIV(1)%NODE_COORDINATES = (/0.0_DP,0.0_DP,0.0_DP,0.0_DP,h,h,h,h/)
  ELEMENT(1)%MESH_COMP(3)%DERIV(5)%NODE_COORDINATES = 1.0_DP
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  DO xi=1,NUMBER_OF_XI
    MESH_COMP_IDX = xi
    FIELD_COMP_IDX = xi
  ...
  DISP_BC%MESH_COMP(1)%NUMBER_OF_BC_NODES_IN_DERIV(7)=4
  DISP_BC%MESH_COMP(1)%NUMBER_OF_BC_NODES_IN_DERIV(8)=4
  ...
  DISP_BC%MESH_COMP(1)%DERIV(3)%NODE_NUMBER(:)=(/1,3,5,7/)
  DISP_BC%MESH_COMP(1)%DERIV(5)%NODE_NUMBER(:)=(/1,3,5,7/)
  DISP_BC%MESH_COMP(1)%DERIV(7)%NODE_NUMBER(:)=(/1,3,5,7/)
  DISP_BC%MESH_COMP(1)%DERIV(8)%NODE_NUMBER(:)=(/1,3,5,7/)
