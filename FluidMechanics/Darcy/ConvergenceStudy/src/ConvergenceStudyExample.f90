...
!-----------------------------------
  ...
  IF( DEBUG ) THEN
    ...
    write(73,*)'TESTCASE: ',TESTCASE

    write(73,*)'STAB: ',STAB
    write(73,*)'ANALYTIC: ',ANALYTIC
    write(73,*)'DEBUG: ',DEBUG

    write(73,*)'LENGTH: ',LENGTH
    write(73,*)'GEOM_TOL: ',GEOM_TOL
    write(73,*)'X1: ',X1
    write(73,*)'X2: ',X2
    write(73,*)'Y1: ',Y1
    write(73,*)'Y2: ',Y2
    ...
    write(73,*)'PERM: ',PERM
    write(73,*)'VIS: ',VIS
    write(73,*)'PERM_OVER_VIS: ',PERM_OVER_VIS

!     write(73,*)'PATH: ',PATH

    write(73,*)'PDEGR: ',PDEGR

    write(73,*)'BC_NUMBER_OF_WALL_NODES: ',BC_NUMBER_OF_WALL_NODES
    write(73,*)'NUMBER_OF_BCS: ',NUMBER_OF_BCS
    write(73,*)' '
  END IF
...
!---------------------------------------------------------
  ...
  !Read nodes, elements and basis information from cmheart input file
  CALL READ_CMHEART_EXE
  !Receive CM container
...
!-----------------
  ...
  CALL FIELD_COMPONENT_VALUES_INITIALISE(MATERIALS_FIELD,FIELD_U_VARIABLE_TYPE,&
    & FIELD_VALUES_SET_TYPE,1,PERM_OVER_VIS,ERR,ERROR,*999)
...
!     CALL EQUATIONS_OUTPUT_TYPE_SET(EQUATIONS,EQUATIONS_ELEMENT_MATRIX_OUTPUT,ERR,ERROR,*999)
  ...
  IF( CM%D==2 ) THEN
    ...
    DO j=1,CM%N_M
      ...
      IF( (ABS(COORD_X-X1) < GEOM_TOL) .OR. &
        & (ABS(COORD_X-X2) < GEOM_TOL) .OR. &
        & (ABS(COORD_Y-Y1) < GEOM_TOL) .OR. &
        & (ABS(COORD_Y-Y2) < GEOM_TOL) ) THEN
  ...
  ELSE IF( CM%D==3 ) THEN
    ...
    DO j=1,CM%N_M
      ...
      IF( (ABS(COORD_X-X1) < GEOM_TOL) .OR. &
        & (ABS(COORD_X-X2) < GEOM_TOL) .OR. &
        & (ABS(COORD_Y-Y1) < GEOM_TOL) .OR. &
        & (ABS(COORD_Y-Y2) < GEOM_TOL) .OR. &
        & (ABS(COORD_Z-Z1) < GEOM_TOL) .OR. &
        & (ABS(COORD_Z-Z2) < GEOM_TOL) ) THEN
  ...
  ALLOCATE(DOF_INDICES( NUMBER_OF_BCS ))
  ALLOCATE(DOF_CONDITION( NUMBER_OF_BCS ))
  ALLOCATE(DOF_VALUES( NUMBER_OF_BCS ))
  ...
  IF( CM%D==2 ) THEN
    ...
    DO j=1,BC_NUMBER_OF_WALL_NODES
      ...
      !
      IF( (ABS(COORD_X-X1) < GEOM_TOL) .OR. (ABS(COORD_X-X2) < GEOM_TOL) ) THEN
        dummy = dummy + 1
        i = 1
      ...
      END IF
      !
      IF( (ABS(COORD_Y-Y1) < GEOM_TOL) .OR. (ABS(COORD_Y-Y2) < GEOM_TOL) ) THEN
        dummy = dummy + 1
        i = 2
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
        IF( TESTCASE == 1 ) THEN
  ...
  ELSE IF( CM%D==3 ) THEN
    ...
    DO j=1,BC_NUMBER_OF_WALL_NODES
      ...
      END IF
      !
      IF( (ABS(COORD_Z-Z1) < GEOM_TOL) .OR. (ABS(COORD_Z-Z2) < GEOM_TOL) ) THEN
        dummy = dummy + 1
        i = 3
        DOF_INDICES( dummy ) = BC_WALL_NODES(j) + (i-1) * CM%N_V
        IF( TESTCASE == 1 ) THEN
  ...
  IF( DEBUG ) CLOSE(73)  !close file for debug output
