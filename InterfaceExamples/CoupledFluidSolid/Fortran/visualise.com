...
for($i=0; $i < 2000; $i=$i+1)
  {
	 $filename = sprintf("./output/Solid/Solid%01d.part0.exnode", $i);
	 print "Reading $filename time $i\n";
	 gfx read node "$filename" time $i;
	 $filename = sprintf("./output/Fluid/Fluid%01d.part0.exnode", $i);
	 print "Reading $filename time $i\n";
	 gfx read node "$filename" time $i;
  }

gfx read elem ./output/Solid/Solid0.part0.exelem;
