...
!       CALL cmfe_Field_GeometricFieldSet(Field,GeometricField,Err)
      ...
      do
        ...
        elseif (trim(adjustl(word))=="#variable") then
          ...
          case (2)
            VariableType=CMFE_FIELD_V_VARIABLE_TYPE
          case (3)
            VariableType=CMFE_FIELD_U1_VARIABLE_TYPE
          case (4)
            VariableType=CMFE_FIELD_U2_VARIABLE_TYPE
          case default
              ...
              do
                ...
                case (CMFE_FIELD_DP_TYPE)
                  ...
                  do i=1,NumberOfComponents
                    ...
                    case (CMFE_FIELD_ELEMENT_BASED_INTERPOLATION)
                      CALL cmfe_Field_ParameterSetUpdateElement(Field,VariableType,CMFE_FIELD_VALUES_SET_TYPE,ind,i,data_dp(i),Err)
                    case (CMFE_FIELD_NODE_BASED_INTERPOLATION)
                      CALL cmfe_Field_ParameterSetUpdateNode(Field,VariableType,CMFE_FIELD_VALUES_SET_TYPE,1,1,ind,i,data_dp(i), &
                        & Err)
                    end select
                  enddo
                case (CMFE_FIELD_INTG_TYPE)
                  ...
                  do i=1,NumberOfComponents
                    ...
                    case (CMFE_FIELD_ELEMENT_BASED_INTERPOLATION)
                      CALL cmfe_Field_ParameterSetUpdateElement(Field,VariableType,CMFE_FIELD_VALUES_SET_TYPE,ind,i,data_int(i), &
                        & Err)
                    case (CMFE_FIELD_NODE_BASED_INTERPOLATION)
                      CALL cmfe_Field_ParameterSetUpdateNode(Field,VariableType,CMFE_FIELD_VALUES_SET_TYPE,1,1,ind,i,data_int(i), &
                        & Err)
                    end select
                  enddo
                end select
