...
! MANUAL BC ASSIGNMENT - REPLACED BY THE ANALYTIC BC ROUTINE BELOW
  ...
  DO NN=1,SIZE(InnerSurfaceNodes,1)
      CALL cmfe_BoundaryConditions_SetNode(BoundaryConditions,DependentField,CMFE_FIELD_DELUDELN_VARIABLE_TYPE,1,1, &
        & InnerSurfaceNodes(NN),abs(InnerNormalXi),CMFE_BOUNDARY_CONDITION_PRESSURE_INCREMENTED,INNER_PRESSURE,Err)   ! INNER_PRESSURE
  ...
  DO NN=1,SIZE(OuterSurfaceNodes,1)
    CALL cmfe_BoundaryConditions_SetNode(BoundaryConditions,DependentField,CMFE_FIELD_DELUDELN_VARIABLE_TYPE,1,1, &
      & OuterSurfaceNodes(NN),abs(OuterNormalXi),CMFE_BOUNDARY_CONDITION_PRESSURE_INCREMENTED,OUTER_PRESSURE,Err)
...
!         CALL cmfe_BoundaryConditions_SetNode(BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABLE_TYPE,1,1,BottomSurfaceNodes(NN),1, &
...
!         CALL cmfe_BoundaryConditions_SetNode(BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABLE_TYPE,1,1,BottomSurfaceNodes(NN),2, &
!           & CMFE_BOUNDARY_CONDITION_FIXED,0.0_CMISSRP,Err)
!         Y_FIXED=.TRUE.
