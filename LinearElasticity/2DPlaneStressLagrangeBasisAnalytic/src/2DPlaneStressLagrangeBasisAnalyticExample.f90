...
PROGRAM ANALYTIC_LINEAR_ELASTICITY_EXAMPLE
  ...
  !Program variables
    INTEGER(INTG) :: NUMBER_GLOBAL_X_ELEMENTS !<number of elements on x axis
    INTEGER(INTG) :: NUMBER_GLOBAL_Y_ELEMENTS !<number of elements on y axis
    INTEGER(INTG) :: NUMBER_GLOBAL_Z_ELEMENTS !<number of elements on z axis
  ...
  !CALL ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT(1,1,1,BASIS_CUBIC_HERMITE_INTERPOLATION,ERR,ERROR,*999)
...
CONTAINS
  ...
  SUBROUTINE ANALYTIC_LINEAR_ELASTICITY_TESTCASE_CONVERGENCE(NUMBER_OF_ELEMENTS_XI_START,NUMBER_OF_ELEMENTS_XI_END, &
    & NUMBER_OF_ELEMENTS_XI_INTERVAL,INTERPOLATION,ERR,ERROR,*)
    ...
    INTEGER(INTG), INTENT(IN) :: INTERPOLATION !<Field Interpolation Specification
    ...
    CALL ENTERS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_CONVERGENCE",ERR,ERROR,*999)
    
    !!TODO:: Set up case structure and update convergence test
    ...
    !ERROR_MESSAGE="The convergence of the bilinear Lagrange Linear Elasticity problem should be between X to X, but it is "//NUMBER_TO_VSTRING( &
    !  & VALUE,"*",ERR,ERROR)//"."
    !CALL TEST_FRAMEWORK_ASSERT_EQUALS(X,VALUE,X,ERROR_MESSAGE,ERR,ERROR,*999)
    
    WRITE(*,'(A)') "Linear Elasticity Analytic Example Testcase1 - bilinear lagrange is successfully completed."
    
    CALL EXITS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_CONVERGENCE")
    RETURN
999 CALL ERRORS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_CONVERGENCE",ERR,ERROR)
    CALL EXITS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_CONVERGENCE")
    RETURN 1
  END SUBROUTINE ANALYTIC_LINEAR_ELASTICITY_TESTCASE_CONVERGENCE

  !
  !================================================================================================================================
  !

  !>Export analytic Linear Elasticity analysis output for a given interpolation
  SUBROUTINE ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT(NUMBER_GLOBAL_X_ELEMENTS,NUMBER_GLOBAL_Y_ELEMENTS, &
    & NUMBER_GLOBAL_Z_ELEMENTS,INTERPOLATION,ERR,ERROR,*)
    ...
    CHARACTER(LEN=100) :: TESTCASE_INTERPOLATION_SPECIFICATION !<The field interpolation Specification string for the given specifications parameter

    CALL ENTERS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT",ERR,ERROR,*999)

    !Solve the Generic Problem
    CALL ANALYTIC_LINEAR_ELASTICITY_GENERIC(NUMBER_GLOBAL_X_ELEMENTS,NUMBER_GLOBAL_Y_ELEMENTS,NUMBER_GLOBAL_Z_ELEMENTS,1,FIELD, &
      ERR,ERROR,*999)
    !Define export analysis filename
    SELECT CASE(INTERPOLATION)
    CASE(BASIS_LINEAR_LAGRANGE_INTERPOLATION)
      TESTCASE_INTERPOLATION_SPECIFICATION = "LINEAR_LAGRANGE_INTERPOLATION"
    CASE(BASIS_CUBIC_HERMITE_INTERPOLATION)
      TESTCASE_INTERPOLATION_SPECIFICATION = "CUBIC_HERMITE_INTERPOLATION"
    CASE DEFAULT
      CALL FLAG_ERROR("Invalid Interpolation Specification.",ERR,ERROR,*999)
    END SELECT
    !Export Analtyic analysis output
    CALL ANALYTIC_ANALYSIS_OUTPUT(FIELD,"ANALYTIC_LINEAR_ELASTICITY_"//TESTCASE_INTERPOLATION_SPECIFICATION,ERR,ERROR,*999)

    CALL EXITS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT")
    RETURN
999 CALL ERRORS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT",ERR,ERROR)
    CALL EXITS("ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT")
    RETURN 1
  END SUBROUTINE ANALYTIC_LINEAR_ELASTICITY_TESTCASE_EXPORT
  ...
  !  & ERR,ERROR,*)
    ...
    TYPE(FIELD_TYPE), POINTER :: GEOMETRIC_FIELD,MATERIAL_FIELD
    ...
    INTEGER(INTG) :: NUMBER_OF_FIELD_VARIABLES, NUMBER_OF_FIELD_COMPS_NUMBER_OF_NODES,NUMBER_OF_FIELD_COMPS, NUMBER_OF_BC_NODES
    ...
    CALL ENTERS("ANALYTIC_LINEAR_ELASTICITY_GENERIC",ERR,ERROR,*999)
    ...
    NULLIFY(ELEMENT)
    ALLOCATE(ELEMENT(NUMBER_OF_ELEMENTS),STAT=ERR)
    IF(ERR/=0) CALL FLAG_ERROR("Could not allocate ELEMENT",ERR,ERROR,*999)
    DO ne=1,NUMBER_OF_ELEMENTS
      ALLOCATE(ELEMENT(ne)%MESH_COMP(NUMBER_OF_MESH_COMPS),STAT=ERR)
    ...
    DO ne=1,NUMBER_OF_ELEMENTS
      DO xi=1,NUMBER_OF_XI
        MESH_COMP_IDX=xi
        ALLOCATE(ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%ELEM_NODE_NUMBERS(MESH_COMP_NUMBER_OF_ELEMENT_NODES(MESH_COMP_IDX)),STAT=ERR)
        ...
        ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%ELEM_NODE_NUMBERS = 0
      ENDDO !xi
    ENDDO !ne
    !-=MODIFIY-HERE-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    !Prescribe Element node numbers
    ELEMENT(1)%MESH_COMP(1)%ELEM_NODE_NUMBERS(:) = (/1,2,3,4/)
    ELEMENT(1)%MESH_COMP(2)%ELEM_NODE_NUMBERS(:) = (/1,2,3,4/)
    !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    DO xi=1,NUMBER_OF_XI
      MESH_COMP_IDX=xi
      DO ne=1,NUMBER_OF_ELEMENTS
        CALL MESH_TOPOLOGY_ELEMENTS_CREATE_START(MESH,MESH_COMP_IDX,XYZ_BASES(xi)%PTR,ELEMENTS,ERR,ERROR,*999)
        CALL MESH_TOPOLOGY_ELEMENTS_ELEMENT_NODES_SET(ne,ELEMENTS,ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%ELEM_NODE_NUMBERS(:), &
          & ERR,ERROR,*999)
        CALL MESH_TOPOLOGY_ELEMENTS_CREATE_FINISH(MESH,MESH_COMP_IDX,ERR,ERROR,*999)
      ENDDO !ne
    ENDDO !xi
    ...
    DO ne=1,NUMBER_OF_ELEMENTS
      DO xi=1,NUMBER_OF_XI
        ...
        DO nu=1,NUMBER_OF_DERIV
          ALLOCATE(ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%DERIV(nu)%NODE_COORDINATES&
          ...
          IF(ERR/=0) CALL FLAG_ERROR("Could not allocate ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%DERIV(nu)%NODE_COORDINATES", &
            & ERR,ERROR,*999)
          ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%DERIV(nu)%NODE_COORDINATES = 0.0_DP
        ENDDO
      ENDDO
    ENDDO
    !-=MODIFIY-HERE-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        DO nu=1,NUMBER_OF_DERIV
          DO np=1,MESH_COMP_NUMBER_OF_ELEMENT_NODES(MESH_COMP_IDX) !Can also loop using nu
            CALL FIELD_PARAMETER_SET_UPDATE_NODE(GEOMETRIC_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,nu, &
              & ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%ELEM_NODE_NUMBERS(np),FIELD_COMP_IDX, &
              & ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%DERIV(nu)%NODE_COORDINATES(np),ERR,ERROR,*999)
          ENDDO !np
        ENDDO !nu
      ENDDO !ne
    ENDDO !xi
        ...
        DO np=1,MESH_COMP_NUMBER_OF_ELEMENT_NODES(MESH_COMP_IDX) !Can also loop using nu
          DO FIELD_COMP_IDX=1,NUMBER_OF_FIELD_COMPS
            CALL FIELD_PARAMETER_SET_UPDATE_NODE(MATERIAL_FIELD,FIELD_U_VARIABLE_TYPE,FIELD_VALUES_SET_TYPE,FIELD_DERIVATIVE_IDX, &
              & ELEMENT(ne)%MESH_COMP(MESH_COMP_IDX)%ELEM_NODE_NUMBERS(np),FIELD_COMP_IDX,MATE_PARA(FIELD_COMP_IDX), &
              & ERR,ERROR,*999)
          ENDDO !FIELD_COMP_IDX
        ENDDO !np
      ENDDO !ne
    ...
    IF(NUMBER_GLOBAL_Z_ELEMENTS/=0) THEN
      ANALYTIC_FUNCTION=EQUATIONS_SET_LINEAR_ELASTICITY_EQUATION_TWO_DIM_PLANE_STRESS_1
    ELSE
      ANALYTIC_FUNCTION=EQUATIONS_SET_LINEAR_ELASTICITY_EQUATION_THREE_DIM_1
    ENDIF
    FIELD_USER_NUMBER = 4 !Dependent Field
    CALL EQUATIONS_SET_ANALYTIC_CREATE_START(EQUATIONS_SET,ANALYTIC_FUNCTION,FIELD_USER_NUMBER,ANALYTIC_FIELD,ERR,ERROR,*999)
    ...
    CALL EXITS("ANALYTIC_LINEAR_ELASTICITY_GENERIC")
    RETURN
999 CALL ERRORS("ANALYTIC_LINEAR_ELASTICITY_GENERIC",ERR,ERROR)
    CALL EXITS("ANALYTIC_LINEAR_ELASTICITY_GENERIC")
    RETURN 1
